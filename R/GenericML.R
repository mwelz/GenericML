...
#' @param fixed_effects Optional vector of fixed-effect group identifiers (for each observation), as generated by \code{\link{make_fixedeffects}()}. The first group-identifier will be internally dropped to avoid perfect collinearity.
...
GenericML <- function(Z, D, Y,
                      ...
                      quantile_cutoffs         = c(0.25, 0.5, 0.75),
                      fixed_effects            = NULL,
                      X1_BLP                   = setup_X1(fixed_effects = fixed_effects),
                      X1_GATES                 = setup_X1(fixed_effects = fixed_effects),
                      diff_GATES               = setup_diff(),
                      diff_CLAN                = setup_diff(),
                      vcov_BLP                 = setup_vcov_defaultFE(fixed_effects),
                      vcov_GATES               = setup_vcov_defaultFE(fixed_effects),
                      monotonize               = TRUE,
  ...
  InputChecks_fixed_effects(fixed_effects, N)
  message_changes()
  ...
  ## if requested, preprocess Z with fixed effects
  # NB: this code chunk must come after setting up Z_CLAN to prevent that
  # possible fixed effects are included in Z_CLAN
  Z <- preprocess_Z(Z = Z, fixed_effects = fixed_effects)


  # render the learners mlr3 environments
  learners <- lapply(seq_along(learners_GenericML),
